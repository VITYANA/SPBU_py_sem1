...
if __name__ == "__main__":
    print("Добро пожаловать в мою игру, в ней я загадаю вам число формата 0000, а вы должны его отгадать. \n"
          "Правила:\n"
    ...
    while guessed_num != hidden_num:
        ...
        for i in range(4):
            if guessed_num[i] in hidden_num:
                if guessed_num[i] == hidden_num[i]:
                    b += 1
                else:
                    a += 1
        print(f'{turn_num}: A - {a} | B - {b}')
    print(f"Верно, поздавляю!\nКоличество попыток - {turn_num}")
